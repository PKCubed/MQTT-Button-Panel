...
static const char *s_menu_items[] = { "Active IP", "MQTT Broker IP", "System Uptime" };
...
static int64_t s_mqtt_connect_time_ms = 0;
...
static bool s_ignore_bank_release[2] = {false, false};
...
static esp_err_t lcd_define_char(uint8_t addr, const uint8_t *pattern)
{
    if (addr > 7) return ESP_ERR_INVALID_ARG;
    esp_err_t err = lcd_send(0x40 | (addr << 3), false);
    if (err != ESP_OK) return err;
    for (int i = 0; i < 8; i++) {
        err = lcd_send(pattern[i], true);
        if (err != ESP_OK) return err;
    }
    return ESP_OK;
}
...
static esp_err_t lcd_init(void)
{
    ...
    // 9. Load custom characters
    static const uint8_t char_up[8] = { 0x04, 0x0e, 0x15, 0x04, 0x04, 0x04, 0x04, 0x00 };
    static const uint8_t char_down[8] = { 0x04, 0x04, 0x04, 0x04, 0x15, 0x0e, 0x04, 0x00 };
    lcd_define_char(0, char_up);   // Can be accessed as \x08
    lcd_define_char(1, char_down); // Can be accessed as \x01
...
static void net_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    if (event_base == ETH_EVENT) {
        ...
        esp_ip4addr_ntoa(&event->ip_info.ip, s_active_ip, sizeof(s_active_ip));
        s_eth_has_ip = true;
...
static void update_system_ui(int touch_preview_idx)
{
    ...
    if (s_in_menu) {
        if (!s_in_submenu) {
            snprintf(line1, sizeof(line1), "%s", s_menu_items[s_menu_index]);
            if (touch_preview_idx == 0) {
                snprintf(line2, sizeof(line2), "Select Option");
            } else {
                line2[0] = '\0';
            }
            
            int len1 = strlen(line1);
            while (len1 < 15) { line1[len1++] = ' '; }
            line1[15] = (s_menu_index > 0) ? '\x08' : ' ';
            line1[16] = '\0';

            int len2 = strlen(line2);
            while (len2 < 15) { line2[len2++] = ' '; }
            line2[15] = (s_menu_index < s_menu_items_count - 1) ? '\x01' : ' ';
            line2[16] = '\0';
        } else {
            if (s_menu_index == 0) {
                snprintf(line1, sizeof(line1), "Active IP:");
                snprintf(line2, sizeof(line2), "%s", s_active_ip);
            } else if (s_menu_index == 1) {
                snprintf(line1, sizeof(line1), "MQTT IP:");
                snprintf(line2, sizeof(line2), "%s", s_mqtt_config.broker_ip);
            } else if (s_menu_index == 2) {
                int64_t uptime_s = (now_ms() - s_boot_time_ms) / 1000;
                int days = uptime_s / 86400;
                int hours = (uptime_s % 86400) / 3600;
                int mins = (uptime_s % 3600) / 60;
                
                int64_t mqtt_s = 0;
                if (s_mqtt_connected && s_mqtt_connect_time_ms > 0) {
                    mqtt_s = (now_ms() - s_mqtt_connect_time_ms) / 1000;
                }
                int m_days = mqtt_s / 86400;
                int m_hours = (mqtt_s % 86400) / 3600;
                int m_mins = (mqtt_s % 3600) / 60;

                if (days > 0) {
                    snprintf(line1, sizeof(line1), "Up: %dd %dh", days, hours);
                } else {
                    snprintf(line1, sizeof(line1), "Up: %dh %dm", hours, mins);
                }
                
                if (s_mqtt_connected) {
                    if (m_days > 0) {
                        snprintf(line2, sizeof(line2), "MQTT: %dd %dh", m_days, m_hours);
                    } else {
                        snprintf(line2, sizeof(line2), "MQTT: %dh %dm", m_hours, m_mins);
                    }
                } else {
            ...
            if (touch_preview_idx == 1) {
                snprintf(line2, sizeof(line2), "Back to Menu");
            }
        }
    ...
    if (s_in_menu) {
        ...
        if (!s_in_submenu) {
            // Main menu: Green for select (Button 0)
            rgb[0][0] = 0; rgb[0][1] = 255; rgb[0][2] = 0;
        } else {
            // Submenu: Red for back (Button 1)
            rgb[1][0] = 255; rgb[1][1] = 0; rgb[1][2] = 0;
        }
            ...
            if (evt.btn_idx == BTN_BANK_DOWN || evt.btn_idx == BTN_BANK_UP) {
                if (evt.type == EVT_BTN_PRESSED) {
                    ...
                    if (s_bank_btns_held[0] && s_bank_btns_held[1]) {
                        s_in_menu = !s_in_menu;
                        s_ignore_bank_release[0] = true;
                        s_ignore_bank_release[1] = true;
                    ...
                    if (s_ignore_bank_release[evt.btn_idx - BTN_BANK_DOWN]) {
                        s_ignore_bank_release[evt.btn_idx - BTN_BANK_DOWN] = false;
                    } else if (!s_in_menu) {
